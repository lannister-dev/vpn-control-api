...
logger = StructuredLogger(logging.getLogger("scenario-enrollment-consumer"))


class ScenarioEnrollmentConsumer:
    ...
    async def start(self):
        if not self._config.enabled:
            logger.info("scenario_enrollment_consumer_disabled")
            return
        ...
        await self._nats.jetstream_subscribe_durable(
            ...
            durable=SCENARIO_ENROLLMENT_DURABLE,
            queue=SCENARIO_ENROLLMENT_DURABLE,
        ...
        self._running = True
        logger.info(
            "scenario_enrollment_consumer_started",
            subject=self._config.notifications_subject,
        )
    ...
    async def stop(self):
        ...
        self._running = False
        logger.info("scenario_enrollment_consumer_stopped")

    async def _handle_message(self, raw_payload: bytes, msg):
        event = self._parse(raw_payload)
        if event is None or event.kind not in SCENARIO_TRIGGERS:
            await msg.ack()
            return
    ...
    @staticmethod
    def _parse(raw_payload: bytes) -> ScenarioTriggerEvent | None:
        try:
            obj = json.loads(raw_payload.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError):
            return None
        try:
            return ScenarioTriggerEvent.model_validate(obj)
