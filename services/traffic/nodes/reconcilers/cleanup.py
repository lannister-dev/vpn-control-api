...
class NodeTrafficHistoryCleanupReconciler(Reconciler):
    name = "node_traffic_cleanup"
    ...
    async def is_enabled(self) -> bool:
        return bool((await self._policy()).node_cleanup_enabled)

    async def interval_sec(self) -> int:
        return max(CLEANUP_IDLE_WHEN_DISABLED_SEC, int((await self._policy()).node_cleanup_tick_sec))

    async def tick(self) -> int:
        return await self._execute_tick((await self._policy()).node_retention_days)
