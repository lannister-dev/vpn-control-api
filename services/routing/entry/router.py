...
def _get_admin_service(session: AsyncSession = Depends(AsyncDatabase.get_session)) -> EntryRoutingAdminService:
...
async def get_state(
    service: EntryRoutingAdminService = Depends(_get_admin_service),
) -> RoutingStateOut:
