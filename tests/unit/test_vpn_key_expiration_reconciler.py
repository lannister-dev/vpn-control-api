...
async def test_no_expired_keys_returns_zero(async_session):
    ...
    with patch("services.vpn.keys.expiration_reconciler.VpnKeyRepository", return_value=mock_key_repo), \
         patch("services.vpn.keys.expiration_reconciler.UserPlacementRepository"), \
         patch("services.vpn.keys.expiration_reconciler.NodeAgentPlacementTransport"):
...
async def test_expired_keys_bulk_revoked(async_session):
    ...
    with patch("services.vpn.keys.expiration_reconciler.VpnKeyRepository", return_value=mock_key_repo), \
         patch("services.vpn.keys.expiration_reconciler.UserPlacementRepository", return_value=mock_placement_repo), \
         patch("services.vpn.keys.expiration_reconciler.NodeAgentPlacementTransport", return_value=mock_transport):
        result = await reconciler._execute_tick()
