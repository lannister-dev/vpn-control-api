...
from services.backend_peers.router import list_backend_peers
...
@pytest.mark.asyncio
async def test_list_backend_peers_router_passes_limit():
    service = MagicMock()
    service.list_peers = AsyncMock(return_value=[])
    backend_node_id = uuid4()
    gateway_node_id = uuid4()

    out = await list_backend_peers(
        backend_node_id=backend_node_id,
    ...
    assert out == []
    service.list_peers.assert_awaited_once_with(
        backend_node_id=backend_node_id,
        gateway_node_id=gateway_node_id,
        limit=654,
    )
...
@pytest.mark.asyncio
async def test_backend_peer_service_passes_limit_to_repository(async_session):
    svc = BackendPeerService(async_session)
    svc.peer_repository = AsyncMock()
    svc.peer_repository.list_active.return_value = []

    out = await svc.list_peers(limit=222)

    assert out == []
    svc.peer_repository.list_active.assert_awaited_once_with(
        backend_node_id=None,
