...
async def test_relay_auth_accepts_valid_bearer(monkeypatch):
    ...
    settings = SimpleNamespace(
        admin=SimpleNamespace(relay_token="relay-secret", relay_token_hash="")
    )
...
async def test_relay_auth_rejects_missing_header(monkeypatch):
    settings = SimpleNamespace(
        admin=SimpleNamespace(relay_token="expected", relay_token_hash="")
    )
    monkeypatch.setattr("services.auth.dependencies.get_settings", lambda: settings)
...
async def test_relay_auth_rejects_when_token_unset(monkeypatch):
    ...
    settings = SimpleNamespace(admin=SimpleNamespace(relay_token="", relay_token_hash=""))
    monkeypatch.setattr("services.auth.dependencies.get_settings", lambda: settings)
...
@pytest.mark.asyncio
async def test_relay_auth_accepts_valid_hash(monkeypatch):
    from services.auth.utils import AuthUtils

    raw = "hash-secret"
    expected_hash = AuthUtils.hash_admin_api_key(raw)
    credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials=raw)
    settings = SimpleNamespace(
        admin=SimpleNamespace(relay_token="", relay_token_hash=expected_hash)
    )
    monkeypatch.setattr("services.auth.dependencies.get_settings", lambda: settings)
...
@pytest.mark.asyncio
async def test_relay_auth_hash_takes_precedence_over_plain(monkeypatch):
    from services.auth.utils import AuthUtils

    raw = "hash-secret"
    expected_hash = AuthUtils.hash_admin_api_key(raw)
    credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials="plain-secret")
    settings = SimpleNamespace(
        admin=SimpleNamespace(relay_token="plain-secret", relay_token_hash=expected_hash)
    )
    monkeypatch.setattr("services.auth.dependencies.get_settings", lambda: settings)
